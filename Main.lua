...
-- Safe file I/O wrappers (protect against executors that restrict file APIs)
local function safe_isfile(fn)
    if type(isfile) ~= "function" then return false end
    local ok, res = pcall(isfile, fn)
    return ok and res
end
local function safe_readfile(fn)
    if type(readfile) ~= "function" then return nil end
    local ok, res = pcall(readfile, fn)
    if ok then return res end
    return nil
end
local function safe_writefile(fn, data)
...
local function LoadSettings(FileName)
  if readfile and isfile and safe_isfile(FileName) then
    local decode = HttpService:JSONDecode(safe_readfile(FileName))
...
local function CreateTween(Configs)
  ...
  local TweenInfo
  local ok, ti = pcall(function() return TweenInfo.new(Time, Theme["Tween Style"]) end)
  if ok and ti then TweenInfo = ti else TweenInfo = TweenInfo.new(Time, Enum.EasingStyle.Quad) end
...
-- ScreenGui
local ScreenGui = Create("ScreenGui", PlayerGui or PlayerGui or CoreGui, {
...
local ScreenFind = (PlayerGui or CoreGui):FindFirstChild(ScreenGui.Name)
...
function redzLib:Destroy()ScreenGui:Destroy()end
function redzLib:MakeWindow(Configs)
  ...
  local function LoadFile()
    if SaveCfg and typeof(SaveCfg) == "string" then
      if readfile and isfile and safe_isfile(SaveCfg) then
        local decode = HttpService:JSONDecode(safe_readfile(SaveCfg))
